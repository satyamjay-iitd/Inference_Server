...
@dataclass
class PinetInference:
	steering_angle: float = 0
	total_pts: int = 0
	num_left_lanes: int = 0
	num_right_lanes: int = 0
	cLy: Optional[any] = None
	cRy: Optional[any] = None
	...
	def __dict__(self):
		d = {
			...
			'TotalPts': self.total_pts,
			'NumLeftLanes': self.num_left_lanes,
			'NumRightLanes': self.num_right_lanes,
			'cLyMax': -1000,
			'cRyMax': -1000,
			'B1': [],
			'B2': []
		}
		if self.cLy is not None:
			d['cLyMax'] = max(self.cLy).item()
			d['cRyMax'] = max(self.cRy).item()
			d['B1'] = self.b1.tolist()
			d['B2'] = self.b2.tolist()
		return d
...
# homography_matrix, homography_matrix_cp = get_homography_matrix()
...
def inference(lane_agent, image, homography_matrix_cp) -> PinetInference:
	...
	for i in range(len(in_x)):
		x1 = cp.array(in_x[i])  # *res_img.shape[1]/512 # scaling of key points to original size of image
	...
	if min_iR != -1 and max_iL != -1:  # IF lane lines are present or not
		...
		if offset > 15:
			...
			steering_angle = -10
		return PinetInference(steering_angle, total_pts, num_left, num_right, xL[1, :], xR[1, :], b1, b2, offset)
	else:  # if no lane line detected then just write total num points detected
		return PinetInference(-100, total_pts, num_left, num_right)
