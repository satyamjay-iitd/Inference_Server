...
class Agent(nn.Module):
                ...
                for point_index, point in enumerate(lane):
                    if point > 0:
                        ...
                        ground[batch_index][2][y_index][x_index] = (target_h[batch_index][lane_index][
                                                                        point_index] * 1.0 / self.p.resize_ratio) - y_index
    ...
    def train_point(self, inputs, target_lanes, target_h, epoch, data_list):
        ...
        for i in range(real_batch_size):
            ...
            node = hard_sampling.sampling_node(loss=hard_loss.cpu().data, data=data_list[i], previous_node=None,
                                               next_node=None)
        ...
        for (confidance, offset, feature) in result:
            ...
            disc_loss = disc_loss + \
                        torch.sum((self.p.K1 - distance_map[ground_truth_instance == 2])[
